...
def backward_general(
    ...
    # # Adding the linear mapping update considering transformation 
    # if trans:
    #     A, lower_b, upper_b = update_trans_A(self.root)
    #     lb = lb + lower_b
    #     ub = ub + upper_b        
    ...
    if need_A_only and self.needed_A_dict and node.name in self.needed_A_dict:
        save_A_record(node, A_record, self.A_dict, self.root, self.needed_A_dict[node.name],
    ...
    if opt_input_poly:
        # NOTE: lb is indeed lbias before concretization
        vol_lb, vol_ub = concretize_poly_vol(lb, ub, node, self.root, batch_size, output_dim,
            bound_lower, bound_upper, average_A=average_A)
        # vol_lb, vol_ub = concretize_poly_vol_LSE(lb, ub, node, self.root, batch_size, output_dim,
        #     bound_lower, bound_upper, average_A=average_A)    
    elif opt_relu_poly:
    ...
    if opt_input_poly:
        vol_lb = vol_lb.view(batch_size, *output_shape) if bound_lower else None
        vol_ub = vol_ub.view(batch_size, *output_shape) if bound_upper else None
    # if opt_relu_poly:
    #     vol_relu_lb = vol_relu_lb.view(batch_size, *output_shape) if bound_lower else None
    #     vol_relu_ub = vol_relu_ub.view(batch_size, *output_shape) if bound_upper else None
    if verbose:
        logger.debug('')

    if self.return_A and (not opt_input_poly) and (not opt_relu_poly):
        return lb, ub, self.A_dict
    elif self.return_A and opt_input_poly:
        return lb, ub, self.A_dict, vol_lb, vol_ub
    elif self.return_A and opt_relu_poly:
        return lb, ub, self.A_dict, vol_relu_lb, vol_relu_ub
    else:
        return lb, ub
...
# xy: for transformation linear mapping
def update_trans_A(root):
    num_inputs = root[0].lA.shape[2]
    upper_d, upper_b, lower_d, lower_b = calc_trans_bound_coefficients(num_inputs)
    # Add an extra specification dimension
    ...
    _lA = root[0].lA
    _uA = root[0].uA
    uA, ubias = calc_trans_by_A_signs(_uA, upper_d, lower_d, upper_b, lower_b)
    lA, lbias = calc_trans_by_A_signs(_lA, lower_d, upper_d, lower_b, upper_b)
    root[0].lA = lA
    root[0].uA = uA
    return [(lA, uA)], lbias, ubias
# get transformation bounding coefficients
def calc_trans_bound_coefficients(num_inputs):
    if preimage_arguments.Config["data"]["dataset"] == 'cartpole':
        # only state-agnostic noise is added 
        upper_d = torch.eye(num_inputs)
        lower_d = torch.eye(num_inputs)
        upper_b = torch.tensor([[0.1, 0, 0, 0]])
        lower_b = torch.tensor([[-0.1, 0, 0, 0]])
    return upper_d, upper_b, lower_d, lower_b
# update the lower and upper bounding A and bias
def calc_trans_by_A_signs(last_A, d_pos, d_neg, b_pos, b_neg):
    if last_A is None:
        return None, 0
    # Compute the transformed linear relaxation coefficients based on signs in last_A.
    _A, _bias = multiply_by_A_signs(last_A, d_pos, d_neg, b_pos, b_neg)
    return _A, _bias
# def calc_trans_by_A_signs(last_A, d_pos, d_neg, b_pos, b_neg, contiguous='auto'):
#     if last_A is None:
#         return None, 0
#     # Compute the transformed linear relaxation coefficients based on signs in last_A.
#     assert isinstance(last_A, Tensor)
...
#     A_trans = d_pos * A_pos + d_neg * A_neg
...
#     return A_trans, bias_pos + bias_neg
