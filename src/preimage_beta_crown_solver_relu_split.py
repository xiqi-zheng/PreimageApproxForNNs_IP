...
class LiRPAConvNet:
    ...
    # NOTE main function for calculating intermediate constraints
    def get_intermediate_constraints(self, split_relu_indices):
        # relu_nodes = [self.get_relu(self.net, i) for i in split_relu_indices]
        relu_nodes = self.net.relus
        A_relu_nodes = [{} for _ in range(len(split_relu_indices))]
        # A_relu_nodes = dict([(relu_idx, {}) for relu_idx in split_relu_indices])
        for i, relu_idx in enumerate(split_relu_indices): # a few things to update
            node = relu_nodes[relu_idx]
            input_node = node.inputs[0]
            A_relu = self.net.compute_intermediate_relu_bounds(input_node, prior_checked=True)
            A_relu_nodes[i].update(A_relu[input_node.name])
        return A_relu_nodes
    ...
    def transfer_to_cpu(self, net, non_blocking=True, opt_intermediate_beta=False, transfer_items="all", bound_lower=True, bound_upper=False):
        ...
        if transfer_items == "all":
            ...
            if bound_upper:
                for cpu_layer, layer in zip(cpu_net.perturbed_optimizable_activations, net.perturbed_optimizable_activations):
                    cpu_layer.uA = layer.uA.to(device='cpu', non_blocking=non_blocking)
                    transfer_size['uA'] += layer.uA.numel()
        # For get_slope().
        ...
        for mi, m in enumerate(self.net.perturbed_optimizable_activations):
            if isinstance(m, BoundRelu):
                ...
                if arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']:
                    ...
                    if arguments.Config["preimage"]["init_beta"]:
                        init_beta = torch.tensor([0.1], dtype=torch.get_default_dtype())
                        m.sparse_beta = init_beta.repeat(batch, max_splits_per_layer[mi])
                    # Load beta from history.
                    for bi in range(batch):
                        if betas is not None and betas[bi] is not None:
                            # First dimension of betas is batch, second dimension is relu layer, third dimension is saved betas.
    ...
    def update_bounds_parallel(
        ...
        with torch.no_grad():
            ...
            if not get_upper_bound:
                ...
                lAs = None
            transfer_net = self.transfer_to_cpu(self.net, non_blocking=False, bound_lower=bound_lower, bound_upper=bound_upper)
        ...
        return ret_l, ret_u, lAs, A, ret_s, ret_b, new_split_history, best_intermediate_betas, primal_x, c
    ...
    def build_the_model(self, input_domain, x, data_lb=None, data_ub=None, vnnlib=None, stop_criterion_func=stop_criterion_sum(0), bounding_method=None, opt_input_poly=False, opt_relu_poly=False):
        ...
        self._set_A_options(get_upper_bound=True)

        # NOTE to record all the symbolic bounds for relus, build preimage polytopes
        ...
        self.net.set_bound_opts({'optimize_bound_args': {
            ...
            'early_stop': False,
            'keep_best': True, 'fix_intermediate_layer_bounds': False,
        ...
        if bounding_method == "alpha-crown":
            ...
            lb, ub, aux_reference_bounds = self.net.init_slope(
                (self.x,), share_slopes=share_slopes, c=self.c, bound_lower=bound_lower, bound_upper=bound_upper)
            ...
            with torch.no_grad():
                ret = self.net.compute_bounds(
                    x=(x,), C=self.c, method='backward', return_A=self.return_A, needed_A_dict=self.needed_A_dict,
                    cutter=self.cutter, bound_lower=bound_lower, bound_upper=bound_upper)
                ...
                if lb is None:
                    lb = torch.zeros_like(ub) - np.inf
                lb, ub, pre_relu_indices = self.get_candidate(self.net, lb, ub)
            return ub[-1], lb[-1], None, None, None, None, None, A, lb, ub, pre_relu_indices, None, None, None
        ...
        # sample_mask = self.get_mask_samples()
        if prune_after_crown_used:
            stime = time.time()
            with torch.no_grad():
                # handle lA
        ...
        return ub[-1], lb[-1], mini_inp, duals, primals, mask, lA, A, lb, ub, pre_relu_indices, slope_opt, history, None
    ...
    def get_lower_bound_naive(
            ...
            bounding_method="crown", C=None, stop_criterion_func=None, bound_lower=False, bound_upper=False):
        batch = len(dm_l)//2
        ...
        if bounding_method == "alpha-crown":
            lb, ub, A_dict = self.net.compute_bounds(
                x=(new_x,), C=C, method='CROWN-Optimized', bound_lower=bound_lower, bound_upper=bound_upper,
                return_A=True, needed_A_dict=needed_A_dict, opt_poly_vol=True)
            # lA = A_dict[self.net.output_name[0]][self.net.input_name[0]]['lA']
        elif bounding_method == 'alpha-forward':
            raise ValueError("Should not use alpha-forward.")
        else:
            with torch.no_grad():
                lb, ub, A_dict = self.net.compute_bounds(x=(new_x,), C=C, method=bounding_method,
        ...
        # return lb, [None] * (batch * 2), ret_s, A_dict
        return lb, ub, ret_s, A_dict
