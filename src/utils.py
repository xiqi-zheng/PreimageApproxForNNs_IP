...
def load_custom_image_dataset(eps_temp=None):
    """
    Load sampled data and define the robustness region
    """
    database_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets/sampled_data') 
    sample_data_dir = arguments.Config["model"]["name"]
    X = np.load(os.path.join(database_path, "{}/X.npy".format(sample_data_dir)))
    mean = 0.1307
    std = 0.3081
    # X = (X - mean) / std    
    labels = np.load(os.path.join(database_path, "{}/y.npy".format(sample_data_dir)))    
    X = torch.from_numpy(X.astype(np.float32))
    labels = torch.from_numpy(labels.astype(int))
    runnerup = np.load(os.path.join(database_path, "{}/runnerup.npy".format(sample_data_dir)))
    runnerup = torch.from_numpy(runnerup.astype(int))
    if eps_temp is None: eps_temp = 0.3
    eps_temp = torch.tensor(eps_temp / std).reshape(1, -1, 1, 1)
    data_max = torch.tensor((1. - mean) / std).reshape(1, -1, 1, 1)
...
def load_verification_dataset(eps_before_normalization):
    ...
    elif "custom" in arguments.Config["data"]["dataset"]:
        data_config = load_custom_image_dataset(eps_temp=eps_before_normalization)
...
def calc_l2_norm(y1, y2):
    ...
    return val 
def calc_l0_spec_sensitive(model, img):
    ...
    for i in range(len(example_idx_list)):
        ...
        if arguments.Config["data"]["num_outputs"] > 1:
            ...
            if arguments.Config["specification"]["robustness_type"] == "verified-acc":
                ...
                new_c = []
                if arguments.Config["data"]["dataset"] == 'dubinsrejoin': # dubinsrejoin is special
                    throttle_label = arguments.Config["preimage"]["runner_up"]
                    for ii in range(num_outputs - 1):
                        # FIXME: we do not need the decision threshold, oh perhaps for some case studies
                        if ii >= 3:
                            spec_idx = ii + 1
                            if spec_idx != throttle_label:
                                c_var = c[:, ii]
                                c_var[0][label] = 0
                                c_var[0][throttle_label] = 1
                                new_c.append((c_var, np.array([arguments.Config["bab"]["decision_thresh"]])))
                        else:
...
def parse_run_mode():
    ...
    if arguments.Config["general"]["csv_name"] is not None and arguments.Config["specification"]["vnnlib_path"] is None:
        ...
        if "Customized" in arguments.Config["data"]["dataset"]:  
            model_ori = load_model(weights_loaded=False)
        else:
            model_ori = load_model()
        if arguments.Config["specification"]["vnnlib_path"] is None:
            ...
            if atk_tp == "l0_sensitive":
                vnnlib_all = construct_vnnlib(X, labels, runnerup, data_max, data_min, perturb_epsilon, target_label, example_idx_list, model_ori)
            else:
