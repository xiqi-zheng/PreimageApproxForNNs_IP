...
def auto_park_2relu():
    return AutoParkModelTwoLayer()
def two_relu_layer_toy_model():
    return ToyModel()
def beta_demo_model():
    return BetaDemoModel()
...
class ToyModel(nn.Module):
    def __init__(self, in_dim=2, hidden_dim=1, out_dim=1):
        super().__init__()
        self.model = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.ReLU(),
        ...
        with torch.no_grad():
            self.model[0].weight.data = torch.tensor([[1., 1.]])
            self.model[0].bias.data = torch.tensor([0.])
            self.model[2].weight.data = torch.tensor([[-1.]])
            self.model[2].bias.data = torch.tensor([2.])
            self.model[4].weight.data = torch.tensor([[1.]])
            self.model[4].bias.data = torch.tensor([-1.])
        print(self.model.state_dict())
    def forward(self, inputs):
        return self.model(inputs) 
   
class BetaDemoModel(nn.Module):
    def __init__(self, in_dim=2, hidden_dim=2, out_dim=1):
        super().__init__()
        self.model = nn.Sequential(
            nn.Linear(in_dim, hidden_dim),
            nn.ReLU(),
        ...
        print(self.model.state_dict())
        with torch.no_grad():
            self.model[0].weight.data = torch.tensor([[1., -1.], [2., -1.]])
            self.model[0].bias.data = torch.tensor([0., 0.])
            self.model[2].weight.data = torch.tensor([[1., -1.]])
            self.model[2].bias.data = torch.tensor([0.])
        print(self.model.state_dict())
    def forward(self, inputs):
        return self.model(inputs)    
...
# Newly trained MLP mnist model
def mnist_3_128():
    """A 3-layer model with 128 hidden neurons"""
    from preimage_model_utils import MnistThreeFeedForward
    model = MnistThreeFeedForward()
    return model
    # model = nn.Sequential(
    #    nn.Linear(784, 128),
    #    nn.ReLU(),
...
class simple_conv_mnist(nn.Module):
    def __init__(self):
        super(simple_conv_mnist, self).__init__()
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1)  # Input channels = 1, Output channels = 16
