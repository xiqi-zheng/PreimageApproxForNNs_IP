...
@torch.no_grad()
def input_split_feature_edge(dm_l_all, dm_u_all, shape=None,
                         cs=None, thresholds=None, split_depth=1, i_idx=None):
    """
    xy: Split the dm_l_all and dm_u_all on input idx with longest edge length for all selected subdomains with split depth as 1.
    """
    dm_l_all = dm_l_all.flatten(1)
    dm_u_all = dm_u_all.flatten(1)
    ...
    assert split_depth == 1 # we only split one depth each step but split on all input features
    # remaining_depth = split_depth # there may be cases where remaining depth 
    input_dim = dm_l_all.shape[1]
    # while remaining_depth > 0:
    ...
    mid = (dm_l_all[indices, idx] + dm_u_all[indices, idx]) / 2

    dm_l_all_tmp[indices, idx] = mid
    dm_u_all_tmp[indices, idx] = mid
    if dm_l_all_cp is None:
        dm_l_all_cp = torch.cat((dm_l_all_tmp, dm_l_all))
        dm_u_all_cp = torch.cat((dm_u_all, dm_u_all_tmp))
    else:            
        dm_l_all_cp = torch.cat((dm_l_all_cp, dm_l_all_tmp, dm_l_all))
        dm_u_all_cp = torch.cat((dm_u_all_cp, dm_u_all, dm_u_all_tmp))
    # remaining_depth -= min(input_dim, remaining_depth)
...
@torch.no_grad()
def input_split_all_feature_parallel(dm_l_all, dm_u_all, shape=None,
                         cs=None, thresholds=None, split_depth=1, i_idx=None):
    """
    xy: Split the dm_l_all and dm_u_all on every possible input idx for all selected subdomains with split depth as 1.
    """
    dm_l_all = dm_l_all.flatten(1)
    dm_u_all = dm_u_all.flatten(1)
    ...
    if cs is not None:
        cs_shape = [2 * input_dim] + [1] * (len(cs.shape) - 1)
        cs = cs.repeat(*cs_shape)
    if thresholds is not None:
        thresholds = thresholds.squeeze(0)
